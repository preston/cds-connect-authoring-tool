...
describe('configPassport', () => {
  ...
  describe('#getLdapConfiguration', () => {
    ...
    beforeEach(async () => {
      ...
      // Use esmock to create a version of configPassport with mocked dependencies
    ...
    it('should skip ca file references that it cannot load', done => {
      // Setup config with mix of good and bad certificate paths
      config.auth.ldap.server.tlsOptions = {
      ...
      getLdapConfiguration(req, (err, ldapConfig) => {
        expect(err).to.be.null;
        expect(ldapConfig.server.tlsOptions).to.eql({
          ca: [Buffer.from('Fake Cert One', 'utf-8')], // Only the good cert should remain
          rejectUnauthorized: true
        });
        done();
      });
  ...
  describe('#getLocalConfiguration', () => {
    ...
    it('should callback with the user when username and passwords match', done => {
      // Test directly with getLocalConfiguration - it will use the real findLocalUserById
      // which reads from local-users.json, so we need actual test users
      getLocalConfiguration('bob', 'p@$$w0rd!', (err, user) => {
        expect(err).to.be.null;
    ...
    it('should callback with the false when passwords do not match', done => {
      getLocalConfiguration('bob', 'wrongpassword', (err, user) => {
        expect(err).to.be.null;
    ...
    it('should callback with the false when user is not found', done => {
      getLocalConfiguration('nonexistentuser', 'anypassword', (err, user) => {
        expect(err).to.be.null;
        expect(user).to.be.false;
        done();
      }, users);
    ...
    it('should callback with error if findLocalUserById returns an error', done => {
      // third argument should be an object but is a string to throw error
      getLocalConfiguration('nonexistentuser', 'anypassword', (err, user) => {
        expect(err).to.be.null;
        expect(user).to.be.false;
        done();
